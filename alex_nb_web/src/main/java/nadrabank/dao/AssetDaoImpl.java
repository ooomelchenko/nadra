...
public class AssetDaoImpl implements AssetDao {
    ...
    public List findAll() {
        List<Asset>list;
        list = factory.getCurrentSession().createQuery("SELECT asset FROM nadrabank.domain.Asset asset LEFT JOIN asset.lot lot LEFT JOIN lot.bid bid ORDER BY bid.bidDate, lot.fondDecisionDate").list();
        return list;
    }
    @Override
    ...
    public List findAll(int portionNum) {
        Query query = factory.getCurrentSession().createQuery("SELECT asset FROM nadrabank.domain.Asset asset LEFT JOIN asset.lot lot LEFT JOIN lot.bid bid ORDER BY bid.bidDate, lot.fondDecisionDate");
    ...
    public List getDecisionNumbers(){
        return factory.getCurrentSession().createQuery("SELECT lot.decisionNumber FROM nadrabank.domain.Lot lot GROUP BY lot.decisionNumber ORDER BY lot.decisionNumber").list();
    }
}